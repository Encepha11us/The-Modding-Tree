class Hopper {
    ...
    isFull() {
        return this.ore.gte(this.capacity);//.mul(0.99999));
    }

    isEmpty() {
        return this.ore.lte(new Decimal(0));//.000001));
    }
...
//effective intake rate = intake rate *


//let s_ratio = 
let s_max = new Decimal(10000)
let s_hoppers = []
s_hoppers.push(new Hopper(new Decimal(100), new Decimal(1), new Decimal(100).div(100), new Decimal(50)))
s_hoppers.push(new Hopper(new Decimal(30), new Decimal(1), new Decimal(20).div(100), new Decimal(30)))
s_hoppers.push(new Hopper(new Decimal(10000000000000), new Decimal(18).div(100), new Decimal(10).div(100), new Decimal(1)))
...
addLayer("s", {
    ...
    buyables: {
        ...
        12: {
            ...
            effect(x) {
                ...
                return new Decimal(10).mul(mult)},
        ...
        13: {
            ...
            effect(x) {
                ...
                mult = mult.mul(x)
                mult = mult.mul(materialToMetalRatio)
                return new Decimal(20).mul(mult)},
        ...
        1201: {
            title: "♈︎ +Capacity",
            effect(x) {
                s_hoppers[0].capacity = s_hoppers[0].capacityBase.mul(x.plus(1));
            },
            cost(x) {return new Decimal(10).pow(x)},
            display() { 
                let data = tmp[this.layer].buyables[this.id]
        ...
        1204: {
            title: "♈︎ +Speed",
            effect(x) {
                s_hoppers[0].cookTime = s_hoppers[0].cookTimeBase.div(x.plus(1));
            },
            cost(x) {return new Decimal(10).pow(x)},
            display() { 
                let data = tmp[this.layer].buyables[this.id]
        ...
        1301: {
            title: "♉︎ +Capacity",
            effect(x) {
                s_hoppers[1].capacity = s_hoppers[1].capacityBase.mul(x.plus(1));
            },
            cost(x) {return new Decimal(10).pow(x)},
            display() { 
                let data = tmp[this.layer].buyables[this.id]
    ...
    automate() {
        ...
        for(let i = 0; i < s_hoppers.length; i++) {
            ...
            if (s_hoppers[i].canCook()) {
                if (s_hoppers[i].isFull()) {
                    ...
                    if (i == 0 ) { //First hopper draws from antimony
                        ...
                        //s_hoppers[i].ore = s_hoppers[i].ore.plus(oreProduced);
                        ...
                        console.log(s_hoppers[i].efficiency);
                        //console.log(s_hoppers[i].intakeRate.toString() + "   " + s_hoppers[i].efficiency.toString());
                        let remainingCapacity = s_hoppers[i].capacity.minus(s_hoppers[i].ore)
                        if (effectiveIntakeRate.gt(remainingCapacity)) {
                            //console.log(effectiveIntakeRate.toString() + " " + remainingCapacity.toString());
                            ...
                            //s_hoppers[i].ore = s_hoppers[i].ore.plus(remainingCapacity);
                            s_hoppers[i].addOre(remainingCapacity);//s_hoppers[i].ore.plus(remainingCapacity));
                        }
                ...
                //else
                    ...
                    //set curcooktime 0
                    //if not empty
                    //transfer our stuff to next hopper
                
                    



            // } else if (i + 1 < s_hoppers.length) {
            //     let oreExpended = transferAmount(s_hoppers[i-1], s_hoppers[i]);
            //     s_hoppers[i-1].ore = s_hoppers[i-1].ore.minus(oreExpended);
            ...
            // } else { //Last hopper gives to our layer's points.
            //     let oreExpended = transferAmount(s_hoppers[i-1], s_hoppers[i]);
            //     let remainingCapacity = s_max.minus(player[this.layer].points);
            //     oreExpended = oreExpended.min(remainingCapacity);
            //     s_hoppers[i-1].ore = s_hoppers[i-1].ore.minus(oreExpended);
